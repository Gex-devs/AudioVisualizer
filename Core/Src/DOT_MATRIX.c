...
uint8_t framebuffer[MAX_CASCADED_NUM][8];
...
void SET_COLUMN(uint8_t au8_MATRIX_Instance, uint8_t au8_Column, uint8_t au8_Value)
{
	...
	uint8_t module_index = au8_Column / 8;
    uint8_t bit_position = au8_Column % 8;

    // Update framebuffer
    for (uint8_t row = 0; row < 8; row++)
    {
        uint8_t bit = (au8_Value >> row) & 0x01;

        if (bit)
            framebuffer[module_index][row] |= (1 << bit_position);   // set bit
        else
            framebuffer[module_index][row] &= ~(1 << bit_position);  // clear bit
    }

    // Send updated rows to the display
    for (uint8_t row = 0; row < 8; row++)
    {
        SS_ENABLE(au8_MATRIX_Instance);

        for (int m = MAX_CASCADED_NUM - 1; m >= 0; m--)
        {
            SPI_TX_Byte(row + 1);  // row 1 to 8
            SPI_TX_Byte(framebuffer[m][row]);
        }

        SS_DISABLE(au8_MATRIX_Instance);
    }
	// uint8_t module_index = au8_Column / 8;
	// uint8_t column_register = (au8_Column % 8) + 1;

	// for (uint8_t i = 0; i < 8; i++)
	// {
	// 	SS_ENABLE(au8_MATRIX_Instance);
	// 	SPI_TX_Byte(0x00); // NO-OP to module 3
	// 	SPI_TX_Byte(0x00);	
	// 	SPI_TX_Byte(0x00);	
	// 	SPI_TX_Byte(0x00);	
	// 	SPI_TX_Byte(i + 1);
	// 	uint8_t mask = 1 << i;
	// 	uint8_t actual_value = au8_Value & mask;
	// 	actual_value >>= i; // get results 
	// 	actual_value <<= au8_Column;
	// 	SPI_TX_Byte(actual_value);
	// 	SPI_TX_Byte(0x00);	
	// 	SPI_TX_Byte(0x00);	
	// 	SS_DISABLE(au8_MATRIX_Instance);
	//} 
